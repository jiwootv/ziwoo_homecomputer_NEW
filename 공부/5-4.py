...
class Game:
    ...
    def load_data(self):
        # 배경그림 불러오기
        self.image_background = pygame.image.load('back.png').convert_alpha()
    ...
    def draw(self):
        ...
        self.draw_text(f'각성 에너지가 내리고 있습니다. {self.player.hit}', 30, pygame.Color('hotpink'), 100, SCREEN_Y * 1 / 20)
        # 구름 그리기
        for cloud in self.clouds:
            cloud.draw()
        # 비 그리기
    ...
    def opening(self):
        ...
        stop = True
        self.draw_text(f'스페이스 바를 누르면 게임이 시작 됩니다.', 30, pygame.Color('white'), 100, SCREEN_Y * 1 / 20)
        self.draw_text(f'구름을 클릭하면 구름이 사라집니다.', 30, pygame.Color('white'), 100, SCREEN_Y * 2 / 20)
        self.draw_text(f'방향키로 상현 1을 조작합니다.', 30, pygame.Color('white'), 100, SCREEN_Y * 3 / 20)
        self.draw_text(f'Q를 누르면 게임이 끝납니다.', 30, pygame.Color('white'), 100, SCREEN_Y * 4 / 20)
        pygame.display.flip()
        while stop:
            self.clock.tick(60)
            pygame.event.get()
